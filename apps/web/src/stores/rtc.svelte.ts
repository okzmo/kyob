...
class RTC {
  currentVC = $state<{ room: Room, serverId: string, channelId: string }>();
  ...
  async connectToRoom(token: string, serverId: string, channelId: string) {
  ...
  async quitRoom() {
    ...
    await this.currentVC.room.disconnect();
  ...
  async toggleMute() {
    ...
    const audioTrack = this.currentVC.room.localParticipant.getTrackPublication(Track.Source.Microphone);
  ...
  async toggleDeafen() {
    ...
    this.currentVC.room.remoteParticipants.forEach((participant) => {
      const audioTrack = participant.getTrackPublication(Track.Source.Microphone);
