...
class Servers {
  ...
  setNewInformations(serverId: string, serverInfos: Partial<Server>) {
    const server = this.getServer(serverId)

    if (serverInfos.name) server.name = serverInfos.name
    if (serverInfos.description) server.description = serverInfos.description
    if (serverInfos.avatar) server.avatar = serverInfos.avatar
    if (serverInfos.banner) server.banner = serverInfos.banner
    if (serverInfos.main_color) server.main_color = serverInfos.main_color
  }
}
