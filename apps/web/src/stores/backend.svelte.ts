...
class Backend {
        ...
        case 'serverChanged':
          {
            if (!wsMess.content.value) return;
            const value = wsMess.content.value;
            const description = new TextDecoder().decode(value.serverInformations?.description);

            const server: Partial<Server> = {
              name: value.serverInformations?.name,
              avatar: value.serverInformations?.avatar,
              banner: value.serverInformations?.banner,
              main_color: value.serverInformations?.mainColor,
            };

            if (description.length > 0) server.description = JSON.parse(description);

            serversStore.setNewInformations(value.serverId, server);
          }
          break;
        case 'connectToCall':
          {
  ...
  async updateServerAvatar(
    ...
    try {
      ...
      const res = await client.post(`server/update_avatar/${serverId}`, {
        body: formData
      });
  ...
  async updateServerProfile(serverId: string, body: UpdateServerProfileType): Promise<Result<void, UpdateAvatarErrors>> {
    try {
      const res = await client.post(`server/update_profile/${serverId}`, {
        body: JSON.stringify(body)
      });
