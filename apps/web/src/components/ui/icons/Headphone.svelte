<script lang="ts">
	interface Props {
		height: number;
		width: number;
		class?: string;
		deafen?: boolean;
	}

	let { class: classes, height = 24, width = 24, deafen = false }: Props = $props();
</script>

{#if deafen}
	<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" {width} {height} class={classes}>
		<path
			fill="currentColor"
			d="M21.488 2.556a.75.75 0 0 0-1.06 0l-1.95 1.95a10 10 0 0 0-10.32-1.66a10 10 0 0 0-6.16 9.22v6.6a3.5 3.5 0 0 0 .51 1.81a.74.74 0 0 0 0 1.06a.7.7 0 0 0 .53.22a.74.74 0 0 0 .53-.22l.52-.52l4.41-4.41l.09-.09l10.49-10.44l2.46-2.46a.75.75 0 0 0-.05-1.06m-13.2 12.14a3.4 3.4 0 0 0-.69-.83a3.47 3.47 0 0 0-4.1-.26v-1.53a8.48 8.48 0 0 1 13.92-6.5zm13.71-2.62v6.6q.008.075 0 .15a3.38 3.38 0 0 1-3.37 3.08a3.39 3.39 0 0 1-3.39-3.19v-2.64a3.4 3.4 0 0 1 1.06-2.31a3.49 3.49 0 0 1 4.22-.33v-1.36a8.7 8.7 0 0 0-.92-3.88a.75.75 0 0 1 .33-1a.77.77 0 0 1 1 .33a10.2 10.2 0 0 1 1.07 4.55"
		/>
	</svg>
{:else}
	<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" {width} {height} class={classes}>
		<path
			fill="currentColor"
			d="M21.998 11.908v6.6a3.4 3.4 0 1 1-6.79 0v-2.64a3.39 3.39 0 0 1 5.29-2.81v-1.15a8.5 8.5 0 1 0-17 0v1.15a3.33 3.33 0 0 1 1.89-.58a3.39 3.39 0 0 1 3.39 3.39v2.64a3.39 3.39 0 1 1-6.78 0v-6.6a10 10 0 0 1 20 0"
		/>
	</svg>
{/if}
