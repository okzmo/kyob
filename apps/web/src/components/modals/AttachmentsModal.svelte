<script lang="ts">
	...
	let attachments = $derived<Attachment[]>(core.attachmentsModal.attachments);
	let maxLimit = $derived(core.attachmentsModal.idx >= attachments.length - 1);
	let minLimit = $derived(core.attachmentsModal.idx <= 0);

	function previous(e: MouseEvent) {
		e.stopImmediatePropagation();
		core.attachmentsModal.idx--;
	}
...
<Dialog.Root
	open={core.attachmentsModal.status}
	onOpenChange={(s) => {
		if (!s) {
			core.closeAttachmentsModal();
...
>
	...
	>
		...
		>
			<img
				src={attachments[core.attachmentsModal.idx].url}
				alt=""
