<script lang="ts">
	...
	let { server, serverName = $bindable(), description = $bindable() }: Props = $props();
	...
	let descriptionText = $derived.by(() => {
		...
		const html = generateHTML(trimEmptyNodes(description || server.description), [
	...
	let needDarkFontColor = $derived(isColorLight(`rgb(${server?.main_color})`));
	...
	async function handleNewAvatar() {
		...
		isSubmitting = true;
		const res = await backend.updateServerAvatar(server.id, parsedData);
...
<Dialog.Root open={openImageModal} onOpenChange={(s) => (openImageModal = s)}>
	...
	<CustomDialogContent>
		<div class="flex items-center justify-between px-8">
			<div>
				<Dialog.Title class="text-lg font-semibold">Change the realm avatar</Dialog.Title>
				<Dialog.Description class="text-main-400 max-w-[24rem] text-sm">
					It will be used as the realm profile banner and avatar.
				</Dialog.Description>
			</div>
		</div>
...
<div
	class="relative z-[2] h-full w-[20rem] shrink-0 overflow-hidden bg-[var(--server-color)] select-none"
>
	<button
	...
	<div class="inner-main-50/10 relative z-[4] flex flex-col px-4 pt-[10.25rem] pb-4">
		...
		<h3 class={['text-xl font-semibold', needDarkFontColor && 'text-main-900']}>{server.name}</h3>
		{#if descriptionText}
			<div
				class={[
					'mt-2 [&>p]:min-h-[24px]',
